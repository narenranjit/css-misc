...
var AjaxConnection = function(url, settings, ajaxOptions){
	...
	function _error(XMLHttpRequestObj,  errorText, errorThrown){
		...
		if(isTimeOutError || isFatalError){
			...
			defaultSettings.onError(errorMessage, errorThrown, responseText);
 ...
 * @param {Object} acOptions to override AjaxConnection
 * @return {this}
 */
var PollingConnection = function(url, conditionFlags, options, acOptions){
	var defaults ={
	...
	var defaultacOptions = {
		tries : 1,
		onSuccess: successHandler,
		parameterParser: null
	}
	$.extend(defaultacOptions, acOptions);
	
	var ac = new AjaxConnection(url, defaultacOptions, jQOptions);
